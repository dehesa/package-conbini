...
extension Publisher {
    ...
    public func sink(fixedDemand: Int, receiveCompletion: ((Subscribers.Completion<Failure>)->Void)?, receiveValue: ((Output)->Void)?) -> AnyCancellable {
    ...
    /// - parameter receiveCompletion: The closure executed when the provided amount of values are received or a completion event is received.
    /// - parameter receiveValue: The closure executed when a value is received.
    public func sink(maxDemand: Subscribers.Demand, receiveCompletion: ((Subscribers.Completion<Failure>)->Void)?, receiveValue: ((Output)->Void)?) -> AnyCancellable {
