...
extension Publisher {
    ...
    ///
    /// If the `test` argument is provided and the publisher doesn't complete before the timeout, the test will fail and a *red debug line* will appear in Xcode.
    ...
    public func expectsCompletion(on test: XCTWaiterDelegate? = nil, timeout: TimeInterval, _ description: String = "The publisher completes successfully", file: StaticString = #file, line: UInt = #line) {
    ...
    /// - parameter file: The file in which failure occurred. Defaults to the file name of the test case in which this function was called.
    /// - parameter line: The line number on which failure occurred. Defaults to the line number on which this function was called.
    public func expectsFailure(on test: XCTWaiterDelegate? = nil, timeout: TimeInterval, _ description: String = "The publisher completes with failure", file: StaticString = #file, line: UInt = #line) {
        precondition(timeout >= 0)
        let exp = XCTestExpectation(description: description)
    ...
    /// Expects the receiving publisher to produce a single value and then complete within the timeout.
    ///
    /// If the `test` argument is provided and the publisher doesn't complete before the timeout, the test will fail and a *red debug line* will appear in Xcode.
    /// - precondition: `timeout` must be greater than or equal to zero.
    /// - parameter test: The test were the expectation shall be fulfilled.
    ...
    @discardableResult public func expectsOne(on test: XCTWaiterDelegate? = nil, timeout: TimeInterval, _ description: String = "The publisher emits a single value and then completes successfully", file: StaticString = #file, line: UInt = #line) -> Self.Output {
        precondition(timeout >= 0)
        let exp = XCTestExpectation(description: description)
    ...
    /// Expects the receiving publisher to produce zero, one, or many values and then complete within the timeout.
    ///
    /// If the `test` argument is provided and the publisher doesn't complete before the timeout, the test will fail and a *red debug line* will appear in Xcode.
    /// - precondition: `timeout` must be greater than or equal to zero.
    /// - parameter test: The test were the expectation shall be fulfilled.
    ...
    @discardableResult public func expectsAll(on test: XCTWaiterDelegate? = nil, timeout: TimeInterval, _ description: String = "The publisher emits zero or more value and then completes successfully", file: StaticString = #file, line: UInt = #line) -> [Self.Output] {
        precondition(timeout >= 0)
        let exp = XCTestExpectation(description: description)
    ...
    @discardableResult public func expectsAtLeast(_ values: Int, on test: XCTWaiterDelegate? = nil, timeout: TimeInterval, _ description: String = "The publisher emits at least the given amount of values and then completes successfully", file: StaticString = #file, line: UInt = #line, foreach check: ((Output)->Void)? = nil) -> [Self.Output] {
