...
extension ResultOpTests {
  ...
  func testCompletionWithoutValue() {
    Empty<Int,_CustomError>(completeImmediately: true).result { _ in
      XCTFail("The handler has been called although it was not expected to")
    }.store(in: &self._cancellables)

    Empty<Int,_CustomError>(completeImmediately: false).result { _ in
      XCTFail("The handler has been called although it was not expected to")
    }.store(in: &self._cancellables)
  }
  ...
  func testFailure() {
    Fail<Int,_CustomError>(error: .init()).result {
