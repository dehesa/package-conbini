...
extension ThenOpTests {
  ...
  func testThenPassthrough() {
    ...
    let cancellable = subject.then {
      ["A", "B", "C"].publisher.setFailureType(to: _CustomError.self)
  ...
  func testThenFailure() {
    ...
    let subject = PassthroughSubject<Int,_CustomError>()
    let cancellable = subject.then {
      Future<String,_CustomError> { (promise) in
    ...
    queue.asyncAfter(deadline: .now() + .milliseconds(100)) { subject.send(completion: .failure(_CustomError())) }
